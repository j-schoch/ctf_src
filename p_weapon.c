...
void Weapon_RocketLauncher_Fire (edict_t *ent)
{
	...
	damage = 100;//js. remove random:   + (int)(random() * 20.0);
	radius_damage = 120;
	damage_radius = 120;
	if (is_quad)
	{
...
BLASTER / HYPERBLASTER / js. VAMPIRE / HEALBLASTER
...
void HealBlaster_Fire (edict_t *ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	fire_healblaster (ent, start, forward, damage, 1000, effect, hyper);
...
void Weapon_HealBlaster_Fire (edict_t *ent)
{
	float	rotation;
	vec3_t	offset;
	int		effect;
	...
	else
	{
		...
		else
		{
			...
			if (deathmatch->value)
				...
				damage = 20;
			HealBlaster_Fire (ent, offset, damage, true, effect);
		}
...
void Weapon_HealBlaster (edict_t *ent)
{
	...
	Weapon_Generic (ent, 5, 20, 49, 53, pause_frames, fire_frames, Weapon_HealBlaster_Fire);
}
//js. vampire methods
void Vampire_Fire (edict_t *ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	vec3_t	forward, right;
	vec3_t	start;
	vec3_t	offset;
	...
	fire_vampire (ent, start, forward, damage, 1000, effect, hyper);
...
void Weapon_Vampire_Fire (edict_t *ent)
{
	float	rotation;
	vec3_t	offset;
	int		effect;
	...
	else
	{
		...
		else
		{
			...
			if (deathmatch->value)
				...
				damage = 10;
			Vampire_Fire (ent, offset, damage, true, effect);
			if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
				ent->client->pers.inventory[ent->client->ammo_index]--;
		}
...
void Weapon_Vampire (edict_t *ent)
{
	...
	Weapon_Generic (ent, 5, 20, 49, 53, pause_frames, fire_frames, Weapon_Vampire_Fire);
}
